...
((SELECT id FROM profiles LIMIT 1), 'order_confirmation', 'New Order', 'Order TT-2024-010 has been placed for ₱720', false),
((SELECT id FROM profiles LIMIT 1), 'low_stock', 'Stock Warning', 'Korean BBQ Wrap stock is below minimum level', false),
((SELECT id FROM profiles LIMIT 1), 'order_confirmation', 'Order Fulfilled', 'Order TT-2024-008 has been completed and ready for pickup', true),
((SELECT id FROM profiles LIMIT 1), 'system_alert', 'Daily Report', 'Today sales total: ₱4,505 from 10 orders', true),
...
((SELECT id FROM profiles LIMIT 1), 'order_confirmation', 'Order Cancelled', 'Order TT-2024-007 was cancelled by customer', true),
((SELECT id FROM profiles LIMIT 1), 'system_alert', 'Peak Hours Alert', 'Lunch rush starting - prepare for high volume orders', false),
((SELECT id FROM profiles LIMIT 1), 'delivery_update', 'Processing Order', 'Order TT-2024-006 is being prepared for delivery', false),
((SELECT id FROM profiles LIMIT 1), 'system_alert', 'Kitchen Update', 'Korean Fried Chicken prep time increased due to high demand', true);